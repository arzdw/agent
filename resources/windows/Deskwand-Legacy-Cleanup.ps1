...
function Write-Step {
  ...
  Write-Host "[Deskwand Cleanup] $Message"
}
...
function Get-OMARegistryEntries {
  ...
  foreach ($glob in $registryGlobs) {
    ...
    $items = Get-ItemProperty -Path $glob -ErrorAction SilentlyContinue | Where-Object {
      $_.DisplayName -like "Deskwand*" -or $_.Publisher -eq "Deskwand Team"
    }
...
function Stop-OMAProcesses {
  $processes = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -ieq "Deskwand.exe" -or $_.ExecutablePath -like "*\Deskwand.exe"
  })

  if ($processes.Count -eq 0) {
    Write-Step "No running Deskwand processes found."
    return
  }
...
Add-UniquePath -List $installPaths -PathValue (Join-Path $env:LOCALAPPDATA "Programs\Deskwand")

$appDataPaths = [System.Collections.Generic.List[string]]::new()
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "Deskwand")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "oma")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "Deskwand")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "oma")

Write-Host ""
Write-Step "This tool removes broken Deskwand Windows install leftovers."
...
if ($failures.Count -eq 0) {
  Write-Step "Cleanup finished. You can rerun the Deskwand installer now."
